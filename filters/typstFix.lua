...
-- convert HTML spans with inline styles to Typst syntax while preserving the color
function Span(span)
    if not FORMAT:match('typst') then return end
    local style = span.attributes.style
    if style then
        local color = style:match("background%-color:%s*([^;]+)")
        if color then
            local typst_style = "highlight(fill: " .. color .. ")"
            local content = pandoc.utils.stringify(span.content)
            return pandoc.RawInline("typst", "#" .. typst_style .. "[" .. content .. "]")
        end
    end
end
