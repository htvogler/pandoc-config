...
local function latex_minipage_align(valign)
  valign = clean_attr(valign)
  if valign == "top" then
    return "t"
  elseif valign == "bottom" then
    return "b"
...
local function build_latex_multipart(div, panels, trailing_blocks)
  ...
  local resolved_panels = resolve_panels(panels)
  local mp_align = latex_minipage_align(valign)

  local out = {}
  local gap_fraction = 0.02
  ...
  if figure_width then
    ...
    if mp_align == "t" then
      table.insert(out, "\\vspace{0pt}")
    end
    table.insert(out, "\\centering")
  end

  local i = 1
  while i <= #resolved_panels do
    local row_panels = {}
    for j = i, math.min(i + cols - 1, #resolved_panels) do
      table.insert(row_panels, resolved_panels[j])
    end

    local row_gap_total = gap_fraction * (#row_panels - 1)
    local usable_fraction = 1.0 - row_gap_total
    ...
    for k, panel in ipairs(row_panels) do
      local pct = pct_values[k]
      local widthspec

      if pct then
        widthspec = string.format("%.4f\\linewidth", usable_fraction * pct / 100)
      elseif has_percent then
        widthspec = string.format("%.4f\\linewidth", usable_fraction * default_pct / 100)
      elseif panel.width then
        widthspec = panel.width
      else
        widthspec = string.format("%.4f\\linewidth", usable_fraction / #row_panels)
      end

      table.insert(out, "\\begin{minipage}[" .. mp_align .. "]{" .. widthspec .. "}")
      if mp_align == "t" then
        table.insert(out, "\\vspace{0pt}")
      end
      table.insert(out, "\\centering")
      ...
      if panel.width and panel.height then
        table.insert(include_opts, "keepaspectratio=false")
      end
      ...
      if k < #row_panels then
        table.insert(out, "\\hspace{" .. string.format("%.4f", gap_fraction) .. "\\linewidth}")
      end
    end
