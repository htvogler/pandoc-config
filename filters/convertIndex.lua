...
-- Check if doindex is true; if not, exit the filter
local doindex = PANDOC_STATE.meta.doindex -- Retrieve doindex from metadata
if not doindex then
  return {} -- Exit the filter without making any changes
end

-- do we Title case the words that go into the index?
local doTitleCase = false
...
local function parseIndexTag(tag)
    local isTerm = false
    local isMain = false
    local indexItem = tag.text

    if string.match("tex", tag.format) then
        -- Match \indext{...} or \index{...} with proper syntax
        if indexItem:match("^\\indext{[^{}]+}$") then
            indexItem = indexItem:match("^\\indext{(.*)}$")
            isTerm = true
        elseif indexItem:match("^\\index{[^{}]+}$") then
            indexItem = indexItem:match("^\\index{(.*)}$")
        else
            return "", false, false -- Not a valid \index or \indext tag
        end

        -- Check for main index marker (*)
        if indexItem:match("^%*") then
            indexItem = indexItem:sub(2)
            isMain = true
        end

        -- Ignore entries with non-standard syntax
        if indexItem:match("%[") or indexItem:match("%]") then
            return "", false, false -- Ignore commands with square brackets
        end
    end
